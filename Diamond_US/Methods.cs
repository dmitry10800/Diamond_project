...
namespace Diamond_US
{
    class Methods
    {
        ...
        {
            var datePat = new Regex(@"(?<month>\w+)\s(?<day>\d+),\s(?<year>\d{4})");
            var a = datePat.Match(v.Trim());
            if (a.Success)
            {
        ...
        {
            Regex patternDate = new Regex(@"(?<month>\d+)\/(?<day>\d+)\/(?<year>\d+)");
            var a = patternDate.Match(tmpDate);
            if (a.Success)
            {
                return "20" + a.Groups["year"].Value + "-" + a.Groups["month"].Value + "-" + a.Groups["day"].Value;
            }
            else
                return tmpDate;
        }
