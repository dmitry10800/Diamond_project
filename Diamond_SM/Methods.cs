...
namespace Diamond_SM
{
    class Methods : ProcessFirstList
    {
        ...
        {
            var prios = new List<ElementOut.PrioStruct>();
            ElementOut.PrioStruct prio;
            var prioList = new List<string>();
            var pat = new Regex(@"(?<number>.*)\s*\b(?<date>\d+\/\d+\/\d{4})\s*(?<country>[A-Z]{2})");
            var tmpV = v.Replace("and", ";");
            if (tmpV.Trim() != "")
            {
        ...
        {
            var inventors = new List<ElementOut.InventorsStruct>();
        ...
        internal static List<ElementOut.ApplicantsStruct> ApplicantsProcess(string s)
        {
            ...
            var applicants = new List<ElementOut.ApplicantsStruct>();
            ElementOut.ApplicantsStruct app;
            var pat = new Regex(@"(?<separator>\sand\s)(?<valueNext>[A-Z]+\b\s*([A-Z]+))");

            var appList = new List<string>();
            if (pat.Match(v).Success)
            {
                var matchCount = pat.Matches(v).Count;
                int startIndex;
            ...
            foreach (var item in appList)
            {
                ...
                var recPat = new Regex(@"(?<name>.*^[^,]+),(?<address>.*),\s*(?<country>([a-zA-z\.]+\s*)+)");
        ...
        internal static ElementOut.AgentStruct AgentProcess(string v)
        {
            ...
            var patA = new Regex(@"(?<name>.*)(?<separator>\sVia\s)(?<address>.*)");
            var patB = new Regex(@"(?<name>.*\,\s*[A-z]+\b)\s(?<address>.*)");
