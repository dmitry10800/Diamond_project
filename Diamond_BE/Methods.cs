...
namespace Diamond_BE
{
    ...
    {
        public static string[] RecSplit(string input)
        {
            string[] splittedRecord = null;

            var ownerMatch = Regex.Match(input, @"\(73\).*?\(73\).*?\((74|54|00)\)", RegexOptions.Singleline);
            var splitByLines = input.Trim().Split('\n');
            var match12 = Regex.Match(input, @"\(00\)\s?(\p{L}+\s?)+");
            var eventDate = "";
            var field12 = "";

            if (splitByLines.Last() != match12.Value)
            {
                field12 = match12.Value.Replace("(00)", "(12)");
                input = input.Replace(match12.Value, field12);
            }

            if (splitByLines.Last().Contains("(00)"))
            {
                eventDate = splitByLines.Last().Replace("(00)", "(99)");
                input = input.Trim().Replace(splitByLines.Last(), "");
                input = input + eventDate;
            }

            if (ownerMatch.Success)
            {
                var owner = ownerMatch.Value.Replace("(74)", "").Replace("(54)", "").Replace("(00)", "");
                input = input.Replace(owner, "");
                var split = owner.Split(new string[] { "(73)" }, StringSplitOptions.RemoveEmptyEntries).Select(x => x.Trim()).ToArray();
                owner = "(73)" + split[0].Replace("\n", " ") + "\n" + split[1];
                input = input.Trim() + "\n" + owner;
            }
            ...
            splittedRecord = input.Split(new string[] { "***" }, StringSplitOptions.RemoveEmptyEntries).Select(x => x.Trim()).ToArray();
