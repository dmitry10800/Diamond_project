...
namespace Diamond_NL
{
    class Methods
    {
        ...
        public static List<string> IntClassProcess(string v)
        {
            var pat = new Regex(@"[A-Z]{1}\d{2}[A-Z]{1}\s*\d+\/\d+");
            return pat.Matches(v).Cast<Match>().Select(x => x.Value.Trim()).Where(x => x != "").ToList();
        }
    }
}
