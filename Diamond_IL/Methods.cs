...
namespace Diamond_IL
{
    class Methods
    {
        ...
        {
            var intClass = new IntClassStruct();
            string classVersion = null;
            var intClassValue = tmpStr;
            var numbersList = new List<string>();
            var versionPattern = @"Cl\.\s*\(\d{4}\.\d{2}\)";
            var numberPattern = @"[A-Z]{1}\d{2}[A-Z]{1}";
            ...
            if (Regex.IsMatch(intClassValue, versionPattern))
            {
                ...
                if (Regex.IsMatch(intClassValue, numberPattern))
                {
                    ...
                    if (splittedNumber != null && splittedNumber.Count() > 0)
                    {
                        ...
                        {
                            var firstPartOfNumber = Regex.Match(item, numberPattern).Value;
                            var tmpItem = item.Replace(firstPartOfNumber, "").Trim();
                            if (tmpItem.Contains(","))
                            {
