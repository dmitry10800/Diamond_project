...
namespace Diamond_GC_OCR
{
    class Methods
    {
        ...
        {
            if (recString.Contains("GC0001666"))
            {

            }
            recString = recString.Replace("Note:", "[99]")
                .Replace("راءات ال", "[99]")
                //.Replace("[11] Patent No.:", "[11]")
                .Replace("Number of the Decision to Grant the Patent:", "[99]");
            var I30 = "[30] Priority:";
            var I31 = "[31] Priority No.";
            var I32 = "[32] Priority date";
            var I33 = "[33] State";
        ...
        {
            var pat = new Regex(@"\d{2}\/[a-zA-Z]+\/\d{4}");
        ...
        public static string DateProcess(string recString)
        {
            ...
            string month = null;
            string year = null;
            var datePattern = @"(?<day>\d+)(\/|\.)(?<month>\d+)(\/|\.)(?<year>\d{4})";
            var datePatternA = @"(?<year>\d{4})(\/|\.)(?<month>\d+)(\/|\.)(?<day>\d+)";
            ...
            if (Regex.IsMatch(recString, datePatternA))
            {
                day = Regex.Match(recString, datePatternA).Groups["day"].Value;
                month = Regex.Match(recString, datePatternA).Groups["month"].Value;
                year = Regex.Match(recString, datePatternA).Groups["year"].Value;
            }
            else
                Console.WriteLine("Pattern date doesn't match!");
        ...
        public static ClassificationInfoStruct ClassificationInfoSplit(string tmpString)
        {
            ...
            var tmpValue = tmpString.Replace("\n", "").Replace(" ", "").Trim();
            ...
            if (splittedRecords != null)
            {
                foreach (var rec in splittedRecords)
                {
                    ...
                    if (rec.Contains(","))
                    {
                        var tmpStringValue = rec.Split(new string[] { "," }, StringSplitOptions.RemoveEmptyEntries);
        ...
        public static InventorStruct InventorSplit(string invString)
        {
            ...
            var tmpValue = invString.Replace("Inventor", "").Trim();
        ...
        {
            var owners = new OwnerStruct();
            var ownerValue = tmpValue.Replace("\n", " ");
            var splittedOwners = new List<string>();
            //Regex ownersPattern = new Regex(@"(?<ownerName>^.*[^,]+)(?<ownerAddress>.*)(\,)(?<ownerCountry>.*$)");
            var separator = new Regex(@"\،\d+\-");
        ...
        public static PriorityStruct PrioritySplit(string prioString)
        {
            ...
            try
            {
                if (tmpPrio != "")
                {
                    var priorityValuesTmp = new PriorityStruct();
                    string[] lineSplittedPrio = null;
                    var datePattern = @"\d+(\/|\.)\d+(\/|\.)\d{4}";
