...
namespace Diamond_VE
{
    class Methods
    {
        ...
        public static AssigneeStruct AssigneeSplit(string str)
        {
            ...
            var I73 = "(73)";
            var tmpStr = str.Replace(I73, "")/*.Replace("\n", " ")*/.Trim();
            /*Multiple (TEST VERSION, need more cases to check!)*/
            var addr = new Regex(@"Domicilio\:", RegexOptions.IgnoreCase);
            var country = new Regex(@"País\:", RegexOptions.IgnoreCase);
            var matchesAddr = addr.Matches(tmpStr);
            var matchesCountry = country.Matches(tmpStr);
            if (matchesAddr.Count > 1 && matchesCountry.Count > 1 && tmpStr.Contains("\n"))
            {

                var testStr = tmpStr.Replace("\n", "****");
                //string tt = Regex.Replace(testStr, @"(País\:)(.*)([A-Z])(\*{4})([A-Z](.*)(Domicilio\:))", "$1$2$3[separator]$5$6");
                var tt = Regex.Replace(testStr, @"(País\:)(.*)([A-Z]|\))(\*{4})([A-Z](.*)(Domicilio\:))", m => m.Groups[1].Value + m.Groups[2].Value + m.Groups[3].Value + "[separator]" + m.Groups[5].Value);
            ...
            if (splValues != null)
            {
                var assignee = new AssigneeStruct();
        ...
        public static ClassificationStruct ClassificationInfoSplit(string tmpString)
        {
            ...
            var tmpClass = tmpString.Replace("(51)", "").Replace("\n", " ").Replace(" ", "").Trim();
        ...
        public static List<PriorityStruct> PrioritySplit(string prioString)
        {
            ...
            var tmpPrio = prioString.Replace("(30)", "").Trim();
            if (tmpPrio == "")
            {
                return null;
            }
